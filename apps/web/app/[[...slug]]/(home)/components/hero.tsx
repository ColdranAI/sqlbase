...
import { Waitlist } from './installer';
...
export const Hero = () => (
  <section className="flex flex-col items-center justify-center gap-6 px-4 my-16 sm:px-16 sm:my-24  border-b border-t border-neutral-800">
    <div className="border-r border-l w-full flex py-10 flex-col justify-center items-center border-neutral-800 ">
    <a
      href="https://x.com/haydenbleasel/status/1929625673586598148"
      target="_blank"
      className="inline-flex group overflow-hidden w-full sm:w-fit items-center gap-2 rounded-full border bg-background py-1 pr-3 pl-1 text-foreground text-sm leading-6 shadow-xs"
      rel="noreferrer"
    >
      <span className="rounded-full group-hover:translate-x-0.5 duration-300 bg-secondary px-2 font-semibold">
        Update
      </span>
      <span className="font-medium truncate">
        SQLBase is looking for users :3
      </span>
    </a>
    <div className="flex flex-col items-center justify-center gap-4">
    <h1 className="max-w-3xl text-balance text-center mt-5 font-semibold text-4xl leading-tight tracking-tighter! sm:text-5xl md:max-w-4xl md:text-6xl lg:leading-[1.1]">
      Visualize your{' '}
    ...
    <p className="max-w-xl text-balance text-center text-muted-foreground md:max-w-2xl md:text-lg">
      A secure playground where you can visualize, interact and analyze your database with a beautiful user interface.
    </p>

    {/* Security Features */}
    <div className="flex flex-col sm:flex-row items-center gap-4 mt-4 text-sm">
      <div className="flex items-center gap-2 px-3 py-2 rounded-full bg-green-900/20 border border-green-800/30">
        <Shield className="w-4 h-4 text-green-400" />
        <span className="text-green-400 font-medium">AES-256-GCM Encryption</span>
      </div>
      <div className="flex items-center gap-2 px-3 py-2 rounded-full bg-blue-900/20 border border-blue-800/30">
        <Lock className="w-4 h-4 text-blue-400" />
        <span className="text-blue-400 font-medium">Enterprise-Grade Security</span>
      </div>
    </div>

    <div className="mx-auto flex w-full max-w-lg mt-5 flex-row justify-center items-center gap-4">
